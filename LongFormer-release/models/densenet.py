...
class DenseNet(nn.Module):
    ...
    def forward(self, x1, x2):
        ...
        img = self.first_conv_1c(x1)
        img = self.features(img)
        flow = self.first_conv_3c(x2)
        flow = self.features(flow)

        # import pdb;pdb.set_trace()

        # features = self.features(x1.flatten(0,1))
        if not self.get_features:
            out = F.relu(img, inplace=True)
            last_duration = int(math.ceil(self.sample_duration / 16))
            last_size = int(math.floor(self.sample_size / 32))
            out = F.avg_pool3d(
                out, kernel_size=(last_size, last_size, last_size)).view(
                    img.size(0), -1)
